    ...
    return (
        <div className='bg-white border border-gray-400 w-[25rem] h-max p-10 mx-auto relative top-20 flex flex-col space-y-8 items-center justify-center rounded-sm'>
            ...
            <section className='flex flex-col items-center'>
                ...
                <p className={`${state.showAnswer ? 'hidden' : ''} break-words w-max`}>{state?.cards?.length === 1 ? state?.cards[0].question : state?.showCard[0]?.question}</p>
                <p className={`${state.showAnswer ? '' : 'hidden'} mb-3`}>Answer:</p>
                <p className={`${state.showAnswer ? '' : 'hidden'} break-words w-max`}>{state?.cards?.length === 1 ? state?.cards[0].answer : state?.showCard[0]?.answer}</p>
            </section>
            ...
            <section className='text-lg w-80 justify-between flex'>
                ...
                <button onClick={toggleAnswer} className='msg-btn w-36'>Edit Card</button>
            </section>    
